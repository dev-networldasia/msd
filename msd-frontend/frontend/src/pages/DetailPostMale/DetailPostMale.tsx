...
import { bg_male, bg_male_detail, male, mobile_post_2, post_1 } from '../../components/ImgExport';
...
const DetailPostMale: React.FC = () => {
  ...
  const listArticleByCategoryOtherArticle = async () => {
    if (Number(categoryDetail?.id)) {
      ...
      // const result = await NewApi.listArticleByUrlCategoryOtherArticle(Number(categoryDetail?.id), String(url), 5, 0)
      if (result.status) {
  ...
  const typeAccumulated = async () => {
    const result = await AccumulateApi.typeAccumulated(tokenUser, 0, 0, 0, 0, 0, (categoryDetail?.id === 4 ? 3 : 0), (categoryDetail?.id === 5 ? 3 : 0), (categoryDetail?.id === 6 ? 3 : 0))
    if (result.status) {
  ...
  const addViewArticle = async (categoryUrl: string | undefined, url: string) => {
    if (categoryUrl && url) {
      navigator(`/chi-tiet-bai-viet-du-phong-cho-nam/${categoryUrl}/${url}`, { state: { keysearch: keysearch } })
      // pushLoading(true)
      // const result = await NewApi.insertViewArticle(id)
      // if (result.status) {
      //   navigator(`/post-detail-male/${categoryId}/${id}`)
      // } else {
      //   pushToast(result?.message || "Đã xảy ra lỗi. Vui lòng thử lại sau!", "warning");
      // }
      // pushLoading(false)
  ...
  const metaHTMLDefault = () => {
    ...
    if (urlTitle) {
      urlTitle.innerHTML = `Chi tiết bài viết dự phòng cho nam/${String(url)}`;
    }
  }
  ...
  const metaHTML = (data: any) => {
    ...
    if (urlTitle) {
      var category
      if (categoryUrl == "du-phong-hpv-cho-nam-tu-9-18-tuoi") {
        category = " từ 9-18 tuổi"
      } else if (categoryUrl == "du-phong-hpv-cho-nam-tu-18-26-tuoi") {
        category = " từ 18-26 tuổi"
      } else if (categoryUrl == "du-phong-hpv-cho-nam-tu-27-45-tuoi") {
        category = " từ 27-45 tuổi"
      } else {
        category = ""
      }
      urlTitle.innerHTML = `Chi tiết bài viết dự phòng cho nam${categoryUrl}/${data ? data.title : 'Dự phòng cho nam'}}`;;
    }
    if (urlMeta) {
      urlMeta.setAttribute("content", `${META_URL}chi-tiet-bai-viet-du-phong-cho-nam/${String(url)}`);
    }
  ...
  // }, [categoryDetail, url, categoryUrl])
  ...
  useEffect(() => {
    if (categoryDetail?.id) {
      ...
      // if (categoryDetail?.id && url) {
      //   addViewNew(Number(categoryDetail?.id), String(url))
      // }
    }
  }, [categoryDetail])
  ...
  return (
    <>
      {/* <Helmet>
        <title>{newDetail ? newDetail.title : 'Dự phòng cho nam'}</title>
      </Helmet> */}
      <div className={showModal ? 'absolute top-0 left-0 w-[100vw] by-svh-custom bg-black z-30 opacity-70' : 'hidden'} onClick={() => setShowModal(false)}></div>
      {/* desktop */}
      <div className="by-svh-custom pt-0 lg:gap-3 xxl:gap-6 hidden lg:flex flex-col justify-between z-40 relative">
        ...
        <div className="by-svh-custom xl:h-full px-5 xxl:px-7 flex flex-col justify-between overflow-y-scroll">
          <div className={`flex ${scroll ? '!h-[507px]' : 'h-[calc(100%-62px)] xl:h-[calc(100%-62px)]'} max-h-[1000px] gap-3 xxl:gap-4`}>
            <div className={`w-2/3 ${scroll ? '!h-[507px]' : 'h-full'} relative`}>
              <div className="w-full h-full bg-white flex flex-col justify-between pt-8 pb-5 pl-8 pr-6 rounded-3xl custom-padding-think">
                ...
                <div className="w-full height-content-male flex gap-4 mt-auto">
                  <div className="w-2/3 overflow-y-scroll custom-scroll-male">
                  ...
                  <div className="w-1/3 gap-4 flex flex-col pr-2 ">
                    ...
                    <div
                      className="text-xs xxl:text-base font-Alexandria text-justify pr-2 overflow-y-scroll custom-scroll-male"
            ...
            <div className={`w-1/3 ${scroll ? '!h-[507px]' : 'h-full'} relative flex xl:py-0 flex-col xxl:pl-3`}>
              <div className="h-1/3 w-full pb-2 xl:pb-4 ">
                <div className="bg-[#045AFA] rounded-3xl h-full w-full flex overflow-hidden transition-all cursor-pointer"
                >
                  <div className="w-[55%] pl-4 xxl:pl-7 py-5 xxl:py-8 flex flex-col justify-between">
                    <div>
                      <p className="mb-0 text-white text-[17px] xxl:text-[19px] font-[500] leading-6 uppercase text-ellipsis line-clamp-3">{categoryDetail?.title}</p>
                    </div>
                    <div className="">
                      <p
                        className="text-white text-xs xxl:text-sm leading-[17px] mt-2 xxl:mt-5 line-clamp-2 xxl:line-clamp-3 text-ellipsis font-Alexandria"
                  ...
                  <div className=" w-[45%] flex flex-col justify-end px-4 pt-5 xxl:pt-8 relative">
                    ...
                    <div className="w-full h-auto flex-shrink-0 overflow-hidden">
                      ...
                      <img
                        className='img'
                        src={categoryDetail?.img ? IMAGE_URL_CATEGORY + categoryDetail?.img : male}
                        onError={(e) => {
              ...
              {listNewOther?.map((item, index) => {
                if (index === 0)
                  return <div key={index} className="h-1/3 w-full py-2"
                    onClick={() => addViewArticle(categoryUrl, item.url)}
                  >
                    <div className=" bg-[#01ABEA] rounded-3xl h-full w-full flex gap-7 overflow-hidden transition-all cursor-pointer"
                    >
              ...
              <div className="grid gap-2 w-full h-1/3  grid-cols-2">
                {listNewOther?.map((item, index) => {
                  if (index === 1 || index === 2)
                    return <div
                      className="bg-white hover:bg-[#7AD6F9] rounded-3xl h-full w-full flex overflow-hidden transition-all cursor-pointer"
                      key={item.id}
                      onClick={() => addViewArticle(categoryUrl, item.url)}
                    >
          ...
          <FooterDesktop colorText='#606060' type='male-post' />
        </div>
      ...
      <div className=' h-full flex lg:hidden flex-col overflow-y-scroll relative'>
        <div className="fixed h-[100vh] w-full top-0 left-0 -z-[1]">
          <img src={bg_male_detail} className='block lg:hidden h-full w-full' alt="hpv" />
        </div>
        <ModalNavigator open={showNotification} dismiss={handleCancel} countDown={countdown} />

        <div className="fixed top-0 left-0 w-full h-10 md:h-[56px] rounded-b-md overflow-hidden z-40 ">

          <img src={bg_male_detail} className='absolute top-0 left-0 w-full h-full object-top -z-10' alt="hpv" />
          {width > 976 ? '' : (<Header handleCancel={handleCancel} handleLink={handleLink} />)}
        </div>
        <div className="pt-12 pb-14 min-h-[100vh] md:pt-16 px-5 xxl:px-7">
          ...
          <div className="lg:hidden flex flex-col h-full">
                  ...
                  >
                    <div className="w-full p-6 flex flex-col gap-[14px]">
                        ...
                        <div className=" flex flex-col gap-1">
                          <div className="text-sm text-left">
                            <figure className=" image image-style-side rounded-3xl overflow-hidden !mt-0">
                              <img
                                className='img '
                                src={newDetail?.img ? IMAGE_URL + newDetail?.img : male}
                                onError={(e) => {
                                  e.currentTarget.src = male;
                                }}
                                alt="hpv"
                      ...
                      <div className="w-full flex flex-col sm:flex-row justify-center items-center gap-2">
                        <a className="uppercase px-6 py-3 text-center flex items-center justify-center rounded-full bg-[#045AF4] text-white text-[15px] font-Alexandria -tracking-[0.3px] cursor-pointer" onClick={() => clickButton('/dia-diem-tu-van')}>
                          TÌM ĐỊA ĐIỂM TƯ VẤN
                        </a>
                        <a className="uppercase px-6 py-3 text-center flex items-center justify-center rounded-full bg-[#045AF4] text-white text-[15px] font-Alexandria -tracking-[0.3px] cursor-pointer"
                          //  onClick={() => navigator('/dia-diem-tu-van', { state: { mylocation: 1 } })}
                          onClick={() => clickButton('handleLink')}
                        >
                          ĐẶT LỊCH VỚI TRUNG TÂM Y TẾ GẦN NHẤT
              ...
              <div className="w-full flex gap-2">
                {listNewOther.length > 0 && listNewOther?.map((item, index) => {
                  if (index === 0 || index === 1)
                    return <div
                      key={item.url}
                      className=" w-1/2 !min-h-[144px] !h-[144px] sm:!h-[169px]"
                      onClick={() => addViewArticle(categoryUrl, item.url)}
                    >
                      <div className=" bg-[#045AF4] rounded-3xl h-full w-full flex gap-4 overflow-hidden transition-all cursor-pointer"
                      >
                  ...
                  if (index > 2) {
                    if (index % 2 === 0) {
                      return <div
                        ...
                        onClick={() => addViewArticle(categoryUrl, item.url)}
                      >
                        <div className=" bg-[#008BEE] rounded-3xl h-full w-full flex gap-4 overflow-hidden transition-all cursor-pointer"
                        >
                      ...
                      return <div
                        ...
                        onClick={() => addViewArticle(categoryUrl, item.url)}
                      >
                        <div className=" bg-[#01ABEA] rounded-3xl h-full w-full flex gap-4 overflow-hidden transition-all cursor-pointer"
                        >
            ...
            <div className="pt-2">
              <Footer colorText='#fff' type='male-post' />
            </div>
