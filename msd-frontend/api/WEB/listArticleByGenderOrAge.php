...
if ($_SERVER['REQUEST_METHOD'] === 'GET') {
                $param = "";
                if($age != ""){
                    $param .= " AND news_category.min <= $age AND news_category.max >= $age";
                }

                if($gender != ""){
                    $param .= " AND news_category.gender = $gender";
                }
                
                $stmt_total = $conn->prepare($getSQL['gNewsByGenderAge'] . $param . " GROUP BY news_matrix.news_id, news_matrix.news_category_id" . " ORDER BY news.id DESC");
                $stmt_total->execute();
                $result_total = $stmt_total->fetchAll(PDO::FETCH_ASSOC);
                $count_total = $stmt_total->rowCount();
                $stmt_total->closeCursor();

                $stmt = $conn->prepare($getSQL['gNewsByGenderAge'] . $param . " GROUP BY news_matrix.news_id, news_matrix.news_category_id" . " ORDER BY news.id DESC" . " LIMIT $limit OFFSET $offset");
                if ($stmt->execute()) {
