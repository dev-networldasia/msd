...
$healthyId = isset($_GET['healthyId']) ? $_GET['healthyId'] : "";
...
if ($_SERVER['REQUEST_METHOD'] === 'GET') {
                $stmt_total = $conn->prepare($getSQL['gHealthyByCategoryOtherHealthy'] . " ORDER BY RAND() DESC");
                $stmt_total->bindValue(":category", $category);
                $stmt_total->bindValue(":healthyId", $healthyId);
                $stmt_total->execute();
                $result_total = $stmt_total->fetchAll(PDO::FETCH_ASSOC);
                $count_total = $stmt_total->rowCount();
                $stmt_total->closeCursor();

                $stmt = $conn->prepare($getSQL['gHealthyByCategoryOtherHealthy'] . " ORDER BY RAND() DESC" . " LIMIT $limit OFFSET $offset");
                $stmt->bindValue(":category", $category);
                $stmt->bindValue(":healthyId", $healthyId);
                if ($stmt->execute()) {
